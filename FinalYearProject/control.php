...
{	
			...
			$sql = "UPDATE status SET status = 1 WHERE deviceID = 1 and sensorID = 6";	// Update present status to database
...
{					
			...
			$sql = "UPDATE status SET status = 0 WHERE deviceID = 1 and sensorID = 6";	// Update present status to database
...
{	
			...
			$sql = "UPDATE status SET status = 1 WHERE deviceID = 2 and sensorID = 9";	// Update present status to database
			// If don't put this If , we can't change the value in database
			if ($conn->query($sql) === TRUE) {	// Because it's been a long time , so i forgot
				//$_GET[] = 1;						// why i have to put this line but it still run when it's commented
			} 
...
{					
			...
			$sql = "UPDATE status SET status = 0 WHERE deviceID = 2 and sensorID = 9";	// Update present status to database
			// Echo "0" , equivalent with send data to App to toast OFF
			if ($conn->query($sql) === TRUE) {	// Because it's been a long time , so i forgot
				//$_GET[] = 0;						// why i have to put this line but it still run when it's commented
			}
...
<form method="POST" action="">
  <div class="control" id="sensor1">
  <p>Device One (1) - Control Relay Module</p>
  <button class="button1" type="submit" name="ON1">ON</button>
  <button class="button2" type="submit" name="OFF1">OFF</button>
</div>

<div class="control" id="sensor2">
  <p>Device Two (2) = Control LED Light</p>
  <button class="button1" type="submit" name="ON2">ON</button>
  <button class="button2" type="submit" name="OFF2">OFF</button>
</div>
</form>
...
<!--<div id="about" class="container text-center">
  ...
  <div class="row">
    ...
    <div class="col-sm-6">
      <p class="text-center"><strong>Profesor Dr. Noor Alamshah</strong></p><br>
